...
const ModalConfirm: FC<Props> = (props) => {
    ...
    return (
        ...
        aria-modal="true"
        >
            <div className={`fixed ${data.visible ? 'grid' : 'hidden'} inset-0 bg-gray-500 bg-opacity-75 transition-opacity`} />
            <div className={`fixed ${data.visible ? 'grid':'hidden'} inset-0 z-10 w-screen overflow-y-auto`}>
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                        ...
                        <div className="sm:flex sm:items-start">
                            ...
                            <svg
                                ...
                                ${
                                    ...
                                    data.type === 'warning'? 'text-yellow-600 ': 
                                    'text-green-600'
                                }`}
                                fill="none"
                                viewBox="0 0 24 24"
                        ...
                        <button
                            ...
                            ${
                                data.type === 'danger'? 'bg-red-600 hover:bg-red-500': 
                                data.type === 'warning'? 'bg-yellow-600 hover:bg-yellow-500': 
                                'bg-green-600 hover:bg-green-500'
                            }`}
                        >
                            {t(data.confirmLabel)} {data.loading ? <Spinner /> : null}
                        </button>
                        <button
