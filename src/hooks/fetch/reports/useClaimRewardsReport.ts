...
import { ClaimRewardsReport, ResponseApi } from "../../../interfaces/reports/ClaimRewardsReportInterface";
import moment from "moment";
import { getData } from "../../models/reports/ClaimRewardsReportModel";
...
const useClaimRewardsReport = () => {
    const [ dataClaimRewardsReport, setDataClaimRewardsReport ] = useState<[][]>()
    const {
        Report,
        Setting
    } = url;
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ClaimRewardsReport)=> getData(Report.claimRewardsReport, data),
        onSuccess: (data:ResponseApi) => {
            if(data.status){
                if(getValues('type')==="view"){
                    setDataClaimRewardsReport(data.data)
                }else{
                    toPdf(data.data??[])
                }
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        for (const value of helperReport.headerClaimRewardReport) {
            newHead=[...newHead,
                t(value)
            ]
        }
    ...
    const onDownload: SubmitHandler<ClaimRewardsReport> = (data) => {
    ...
    const onSubmit: SubmitHandler<ClaimRewardsReport> = (data) => {
        mutate({
            ...data,
        })
        setValue('type', 'view')
...
export default useClaimRewardsReport;