...
import { PointsReport, ResponseApi } from "../../../interfaces/reports/PointsReportInterface";
import moment from "moment";
import { getData } from "../../models/reports/PointsReportModel";
...
const usePointsReport = () => {
    const [ dataPointsReport, setDataPointsReport ] = useState<[][]>()
    const {
        Report,
        Setting
    } = url;
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:PointsReport)=> getData(Report.pointsReport, data),
        onSuccess: (data:ResponseApi) => {
            if(data.status){
                if(getValues('type')==="view"){
                    setDataPointsReport(data.data)
                }else{
                    toPdf(data.data??[])
                }
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        for (const value of helperReport.headerPointsReport) {
            newHead=[...newHead,
                t(value)
            ]
        }
        ...
        doc.save(`${t('points report')}.pdf`)
    }

    const onDownload: SubmitHandler<PointsReport> = (data) => {
    ...
    const onSubmit: SubmitHandler<PointsReport> = (data) => {
        mutate({
            ...data,
        })
        setValue('type', 'view')
...
export default usePointsReport;