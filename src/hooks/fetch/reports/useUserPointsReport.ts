...
import { UserPointsReport, ResponseApi } from "../../../interfaces/reports/UserPointsReportInterface";
import moment from "moment";
import { getData } from "../../models/reports/UserPointsReportModel";
...
const useUserPointsReport = () => {
    const [ dataUserPointsReport, setDataUserPointsReport ] = useState<[][]>()
    const {
        Report,
        Setting
    } = url;
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:UserPointsReport)=> getData(Report.userPointsReport, data),
        onSuccess: (data:ResponseApi) => {
            if(data.status){
                if(getValues('type')==="view"){
                    setDataUserPointsReport(data.data)
                }else{
                    toPdf(data.data??[])
                }
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        for (const value of helperReport.headerUserPointsReport) {
            newHead=[...newHead,
                t(value)
            ]
        }
        ...
        doc.save(`${t('UserPoints report')}.pdf`)
    }

    const onDownload: SubmitHandler<UserPointsReport> = (data) => {
    ...
    const onSubmit: SubmitHandler<UserPointsReport> = (data) => {
        mutate({
            ...data,
        })
        setValue('type', 'view')
...
export default useUserPointsReport;