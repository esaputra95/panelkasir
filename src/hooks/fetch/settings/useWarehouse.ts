...
export const useWarehouse = () => {
    ...
    const {
        ...
        networkMode: 'always',
        queryFn: async () => await getData(Warehouse.get, 
            {
                ...query, 
                page:page.page,
                limit: page.limit
    ...
    const optionWarehouse = async (data: string): Promise<OptionSelectInterface[]> => {
        const response = await getDataSelect(Warehouse.getSelect, {name: data});
        if(response.status){
            setDataOptionWarehouse(response.data.warehouse);
    ...
    const { mutate:mutateById } = useMutation({
        mutationFn: (id:number) => getDataById(Warehouse.getById, id),
        onSuccess:(data:ApiResponseUpdateWarehouse)=>{
            if(data.status){
                const value = data.data.Warehouse
                reset({
                    id: value.id,
                    name: value.name,
                    phone: value.phone,
    ...
    const { mutate } = useMutation({
        mutationFn: async (data:WarehouseInterface)=> {
            return await postData(Warehouse.post, data)
        },
        onSuccess: () => {
            setModalForm((state)=>({
                ...state,
    ...
    const {mutate:mutateDelete} = useMutation({
        mutationFn: (id:number) => deleteData(Warehouse.delete, id),
        onSuccess: () => {
            modalConfirm.setModalConfirm({
                ...modalConfirm.modalConfirm,
                loading: false,
