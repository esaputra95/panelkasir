...
const useAttendanceReport = () => {
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportAttendance.report, {
    ...
    const { mutate:mutatePdf, isLoading:isLoadingMutateExcel } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportAttendance.excel, {
            startDate: data.startDate,
            endDate: data.endDate,
            storeId: data.warehouse?.value,
        }),
        onSuccess: async () => {
            await downloadFile(ReportAttendance.download, 'Laporan Kehadiran')
            toast.success(t("success-get-data"), {
                position: toast.POSITION.TOP_CENTER
            });
        },
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        const headers = helperReport.headerAttendanceReport;
        ...
        doc.save(`${t('attendance report')}.pdf`)
    }
