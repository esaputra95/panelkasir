...
const useCashOutReport = () => {
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportCashOut.report, {
    ...
    const { mutate:mutatePdf, isLoading:isLoadingMutateExcel } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportCashOut.excel, {
            startDate: data.startDate,
            endDate: data.endDate,
            storeId: data.warehouse?.value,
        }),
        onSuccess: async () => {
            await downloadFile(ReportCashOut.download, 'Laporan Kas Keluar')
            toast.success(t("success-get-data"), {
                position: toast.POSITION.TOP_CENTER
            });
        },
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        const headers = helperReport.headerCashOutReport;
        ...
        doc.save(`${t('cash out report')}.pdf`)
    }
...
export default useCashOutReport;
