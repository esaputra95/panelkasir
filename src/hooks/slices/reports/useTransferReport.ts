...
const useTransferReport = () => {
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportTransfer.report, {
    ...
    const { mutate:mutatePdf, isLoading:isLoadingMutateExcel } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportTransfer.excel, {
            startDate: data.startDate,
            endDate: data.endDate,
            storeId: data.warehouse?.value,
        }),
        onSuccess: async () => {
            await downloadFile(ReportTransfer.download, 'Laporan Transfer')
            toast.success(t("success-get-data"), {
                position: toast.POSITION.TOP_CENTER
            });
        },
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        const headers = helperReport.headerTransferReport;
        ...
        doc.save(`${t('transfer report')}.pdf`)
    }
