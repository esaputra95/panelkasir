...
const useAccountBalanceReport = () => {
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportAccountBalance.report, {
    ...
    const { mutate:mutatePdf, isLoading:isLoadingMutateExcel } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportAccountBalance.excel, {
            startDate: data.startDate,
            endDate: data.endDate,
            storeId: data.warehouse?.value,
        }),
        onSuccess: async () => {
            await downloadFile(ReportAccountBalance.download, 'Laporan Saldo Akun')
            toast.success(t("success-get-data"), {
                position: toast.POSITION.TOP_CENTER
            });
        },
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        const headers = helperReport.headerAccountBalanceReport;
        ...
        doc.save(`${t('account balance report')}.pdf`)
    }
...
export default useAccountBalanceReport;
