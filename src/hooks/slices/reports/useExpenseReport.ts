...
const useExpenseReport = () => {
    ...
    const { mutate, isLoading:isLoadingMutate } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportExpense.report, {
    ...
    const { mutate:mutatePdf, isLoading:isLoadingMutateExcel } = useMutation({
        mutationFn: (data:ReportBase)=> getData(ReportExpense.excel, {
            startDate: data.startDate,
            endDate: data.endDate,
            storeId: data.warehouse?.value,
        }),
        onSuccess: async () => {
            await downloadFile(ReportExpense.download, 'Laporan Biaya')
            toast.success(t("success-get-data"), {
                position: toast.POSITION.TOP_CENTER
            });
        },
    ...
    const toPdf = async (data:string[][]) => {
        ...
        let newHead:string[]=[];
        const headers = helperReport.headerExpenseReport;
