...
import { SchoolYearInterface } from "../../../../interfaces/settings/schoolYearInterface";
import { useTranslation } from "react-i18next";
import Skeleton from "../../../../components/ui/Skeleton";
import moment from 'moment';

type tableProps = {
    data?: SchoolYearInterface[],
    isFetching?: boolean,
    page: number,
    limit: number,
                ...
                <tbody>
                    {
                        !isFetching && data && data.length > 0 ? data.map((value, index)=>(
                            <tr key={value.id} className="overflow-auto bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td className="px-6 py-4">
                                    { moment(value.startYear).format('DD/MM/YYYY')}
                                </td>
                                <td className="px-6 py-4">
                                    { moment(value.endYear).format('DD/MM/YYYY')}
                                </td>
                                <td className="px-6 py-4">
                                    {value.description}
                                </td>
