...
import { BankTableInterface } from "../../../../interfaces/masters/BankInterface";
import { useTranslation } from "react-i18next";
import Skeleton from "../../../../components/ui/Skeleton";

type TableProps = {
    data?: BankTableInterface[],
    isFetching?: boolean,
    page: number,
    limit: number,
                ...
                <tbody>
                    {
                        !isFetching && data && data.length > 0 ? data.map((value, index)=>(
                            <tr key={value.id} className="overflow-auto bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`p-1 rounded-md ${value.status==="draft" ?  'bg-orange-100 text-orange-800': 'bg-green-100 text-green-800'}`}>{value.status}</span>
                                </td>
                                <td className="px-6 py-4">
                                    {value.description}
                                </td>
